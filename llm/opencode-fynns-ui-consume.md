...
2. Follow installer + hard rules (**zero-token sibling** `../fynns_ui_design_core` + `file:` → `@fynn7/ui-design-core`, Vite alias `@fynns/ui`, ES2022+, **API-only** — props/children; no consumer restyle). **No** `NODE_AUTH_TOKEN` for day-to-day. Purge table: `llm/BREAKING_PURGE.md`. No git submodule for day-to-day consume.
