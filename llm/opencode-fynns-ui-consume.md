...
3. For component APIs and preview anatomy, use `AGENTS.md` and `examples/sandbox/src/pages/*PreviewCanvas.tsx`. Chat containers must stay ≥ `--fynns-radius-22` unless the user explicitly asks squarer.
4. For **shells / inspectors / catalogs / live token drafts**, also read `llm/PERF.md` and apply its DoD before coding. For `ClippedNavShell`, keep `navMode` in sync with Drawer vs Rail children — see `llm/CONSUMER_TREATY.md` / `llm/consumer-cursor-rule.mdc` (never rail-width + labeled drawer).
