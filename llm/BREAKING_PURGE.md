...
| `NavigationDrawer` destination gap | Consecutive Item / Group / Headline / Divider siblings share `--fynns-navdrawer-section-gap` (**4dp**). Non-destination chrome (SearchBar tools) as a body sibling opens `--fynns-layout-unit-stack-gap` (**16dp**) to the first destination — body is no longer a uniform 4dp flex gap. Do **not** wrap Item / Group / Headline lists in `.fynns-unit-stack` (16dp form rhythm). Core remaps `.fynns-nav-drawer-body .fynns-unit-stack` to `section-gap` so mistaken destination wraps still match Group rows — prefer direct children. Re-paste `consumer-cursor-rule.mdc`. |
