...
| `ChatMessage` streaming bubble / caret | `streaming` with empty/whitespace `children` paints **no** answer bubble and **no** caret (thinking / activity can still show above). Row stays `aria-busy`. Caret only while streaming **and** answer text exists. Drop consumer loaders that assumed an empty bubble / lone caret during thinking-only wait. |
