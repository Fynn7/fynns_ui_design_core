...
| `NavigationDrawer` destination gap | Body siblings (Item / Group / Headline / Divider / destination-density SearchBar / tools) share `--fynns-navdrawer-section-gap` (**4dp**) — including SearchBar ↔ first Item. Do **not** wrap Item / Group / Headline lists in `.fynns-unit-stack` (16dp form rhythm). Core remaps `.fynns-nav-drawer-body .fynns-unit-stack` to `section-gap` so mistaken destination wraps still match Group rows — prefer direct children. Re-paste `consumer-cursor-rule.mdc`. |
