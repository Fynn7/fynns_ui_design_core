...
| `Pagination` page strip | The page list is **nowrap** and the nav owns the **full host width**. Page-size Select / “Showing…” copy must be the **previous Card-body sibling** (unit-stack), not a horizontal space-between partner. Re-paste `consumer-cursor-rule.mdc`. |
