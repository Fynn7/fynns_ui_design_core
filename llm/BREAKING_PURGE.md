...
| BusyRegion sectional scrim (≥ **0.5.121**) | `.fynns-busy-region-overlay` paints `var(--fynns-color-overlay)` — dims mounted children so busy copy does not stack on legible mono / catalog text. **Not** the old `surface-1` wash and **not** `background: transparent`. Do **not** add consumer scrim CSS. Live: `#busy-region` FieldBlock refresh over `CodeBlock`. Re-paste `consumer-cursor-rule.mdc`. |
