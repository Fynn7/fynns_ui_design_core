...
| Card / Collapsible title lead (≥ this ship; **yield** ≥ **0.5.57**) | `.fynns-card-lead` / `.fynns-collapsible-trigger` prefer `flex: 1 1 40%` with **`min-width: 0`** (not a hard `40%` floor) so dense header actions stay fully visible under Card `overflow: hidden`; title ellipsizes. Prefer one flat `.fynns-control-cluster` of IconButtons in `actions` — **no** `ToggleGroup` in the head (body `ControlRow`). Live: sandbox `#card` actions-strip. |
