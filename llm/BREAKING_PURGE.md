...
| `UnitStack` | `.fynns-unit-stack` or Card / Collapsible body (built-in `unit-stack-gap`). Control + its note → `ControlBlock`, not a revived stack component. Related form fields / preference switches by **semantic kind** → `FieldStack` (not a flat Card-body list). Sandbox demos may still use `.sandbox-stack`. |
...
| Editable `CodeBlock` height | Default **`autoGrow`** (content-sized from `rows` floor `1` up to `maxHeight`). Was a fixed tall `rows` well in demos. Pass `autoGrow={false}` for a fixed well or fill hosts that set `textarea { height: 100% }`. |
...
| Overlay head\|body hairline | Centered `Dialog` / `ConfirmDialog`, `Drawer`, `BottomSheet`, `FullscreenDialog`, and Date/Time picker dialogs paint **no** head\|body `border-strong` divider and no `surface-head` title strip (single surface; Sheet keeps the drag handle). Do not restyle a Card-like hairline back onto these heads. Centered Dialog pads: non-confirm head block-start `dialog-inset/2` + end `0`; Confirm head block-start full `dialog-inset`; `head + body` pad-top `space-md`; foot block-end `space-lg` (was full `dialog-inset` under the hit box). |
...
| Card / Collapsible `chrome="card"` body | Bodies are flex columns with `--fynns-layout-unit-stack-gap` (**16dp**) between **direct** children. Drop consumer `margin-bottom` on FieldBlock / intro copy that doubles the gap. Related control + narrative → `ControlBlock` (`description`), not a loose muted `<p>` after `ControlStack`. Consecutive same-kind form units → `FieldStack` (`field-stack-gap` 8dp); adjacent FieldStacks → `form-cluster-gap` (**32dp**). `--fynns-layout-field-hint-gap` is **8dp** (no longer aliases unit-stack). |
