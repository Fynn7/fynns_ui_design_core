...
| `List` `--with-end` overlay reveal + path trailing md (≥ 0.4.107) | End action clusters on `--with-end` rows **overlay-reveal** (idle full-bleed copy; hover / focus-within shows trailing; touch always visible). Path / link catalogs use default **`md`** IconButtons — drop `size="sm"` on List trailing when a sibling catalog `ControlRow` uses `md`. `--fynns-list-height-2` **60dp → 56dp** (`3.5rem`). Live: sandbox `#list` / `#rhythm`. Re-paste `consumer-cursor-rule.mdc`. |
