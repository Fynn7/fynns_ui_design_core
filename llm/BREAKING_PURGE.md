...
| Experience / job List meta (≥ 0.4.118; start column token 0.4.119; **opt-in `trailingMetaAlign` ≥ 0.4.120**; **1-icon `--with-end` reserve ≥ 0.4.121**; end-ink **0.4.133**; **start-ink ≥ 0.4.144**) | Organization under title; dates → `trailingSupportingText`. Mixed-length date / timestamp catalogs → **`List` `trailingMetaAlign="start"`** (shared min-width column; **start**-aligned copy ≥ 0.4.144). Short status + `--with-end` action catalogs **omit** the prop. Live: `#list` org+dates / run-summary / status+action. Re-paste `consumer-cursor-rule.mdc`. |
...
| List `trailingMetaAlign` column end-align (≥ **0.4.133**; **superseded start-ink ≥ 0.4.144**) | 0.4.133 briefly end-aligned copy inside the 17ch column. **≥ 0.4.144** reverses to **start**-aligned ink (same grammar as headline duration / trailing-stats). Keep `trailingMetaAlign="start"` on date / timestamp catalogs. Live: `#list` org+dates / run-summary. |
| List meta column start-ink (≥ **0.4.144**) | `trailingMetaAlign="start"` + `.fynns-list-item-trailing-stats` cells **start**-align. Drop consumer private `text-align` / width hacks. Live: `#list` org+dates / run-summary / usage-stats. |
...
| List run-summary single line (≥ **0.4.141**; gap **0.4.142**; meta start **0.4.143**; trailing start **0.4.144**; overflow **0.4.145**) | Status + identity + duration on **one** horizontal `ListItem` line. Cluster gap = `control-stack-gap` **8dp**. Headline `.fynns-table-meta` = fixed elapse+icon track + **start** align (≥ 0.4.143); **one metric per cell** + label `<span>` ellipsis (≥ 0.4.145). Timestamp → `trailingSupportingText` + **`List` `trailingMetaAlign="start"`** (start ink ≥ 0.4.144). Drop `InlineAlert`, `lines={2}` stack, between-row `Divider`, jammed multi-metric meta, private status↔title margin, and content-width / end-ink meta. Live: `#list` run-summary. |
