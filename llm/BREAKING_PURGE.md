...
| `Pagination` page strip | The page list is **nowrap** and the nav is **content-width** (`max-content`, capped at host). Page-size Select / “Showing…” copy must be the **previous Card-body sibling** (unit-stack), not a horizontal space-between partner. Re-paste `consumer-cursor-rule.mdc`. |
| `ListItem` interactive trailing | On interactive **and** static (`interactive={false}`) rows, `trailing` (IconButtons / menus) is a **sibling** of the row control (valid nesting; shares host `radius-3xl` state-layer). Meta `trailingSupportingText` stays inside the row control. Path / link catalogs: one `List` — never `Surface`/`Card` per entry. Re-paste `consumer-cursor-rule.mdc`. |
...
| `DestinationAppShell` densify rail labels (≥ 0.4.64) | Default `railLabelVisibility` is **`unlabeled`** (Cursor icon column + footer gear — no stacked caption pills). Pass `railLabelVisibility="labeled"` when densified captions must stay visible. Re-paste `consumer-cursor-rule.mdc`. |
