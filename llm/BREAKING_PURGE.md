...
| CodeBlock editable selection / wrap (≥ 0.4.88; soft-wrap single textarea ≥ **0.5.27** — visual-line overlay **0.5.23–0.5.26 removed**) | **`readOnly`** → single `.fynns-code-block-pre` (full tokens + native `::selection`). Soft-wrap **edit** (`wrap` default) → **single visible textarea** + native `::selection` (no live token overlay — Chromium textarea≠pre wrap breakpoints). **Live token colors while typing** → `wrap={false}` (classic dual-layer `<pre>`). Live: Globals `#code-block` soft-wrap sample + file-body + readOnly pair. |
