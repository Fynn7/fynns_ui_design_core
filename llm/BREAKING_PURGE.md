...
| `ClippedNavShell` narrow drawer (≥ 0.4.83) | Viewport ≤56.25rem **no longer** stacks a rem-capped (`9rem`) labeled drawer above main. Destinations stay a side column; `onNavCrowded` fires on that breakpoint when `navMode="drawer"`. Apps densify to `"rail"` + `NavigationRail` (or `DestinationAppShell`). Live: `#layouts-demo-shell`. |
