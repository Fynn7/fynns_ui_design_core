...
| `ChatMessage` body sibling stack | Adjacent **element** children get `--fynns-chatmessage-body-stack-gap` (**16dp**). Bare string / number children are promoted to `.fynns-chat-message-prose` so `{text}<CodeBlock/>` stacks without a consumer wrapper. Do **not** invent app CSS for this gap; drop one-off `<div>{text}</div>` workarounds that only existed to force element siblings. Streaming caret nests in the last prose when that unit is trailing. |
