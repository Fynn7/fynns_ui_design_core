...
| CodeBlock editable selection / wrap (≥ 0.4.88; continuous soft-wrap wash ≥ **0.4.139**) | **`readOnly`** → single `.fynns-code-block-pre` (full tokens + native `::selection`). Soft-wrap **edit** overlay uses flat mono highlight (no token spans) so selection/caret stay aligned; selection wash is `.fynns-code-block-sel` (no dark seams between wrap fragments). `wrap={false}` for live token colors while typing. Live: Globals `#code-block` soft-wrap sample + file-body + readOnly pair. |
...
| Experience / job List meta (≥ 0.4.118; start column token 0.4.119; **opt-in `trailingMetaAlign` ≥ 0.4.120**; **1-icon `--with-end` reserve ≥ 0.4.121**; end-ink **0.4.133**; start-ink **0.4.144** **superseded end-ink ≥ 0.4.148**) | Organization under title; dates → `trailingSupportingText`. Mixed-length date / timestamp catalogs → **`List` `trailingMetaAlign="start"`** (shared min-width column; **end**-aligned copy ≥ 0.4.148). Short status + `--with-end` action catalogs **omit** the prop. Live: `#list` org+dates / run-summary / status+action. Re-paste `consumer-cursor-rule.mdc`. |
...
| List `trailingMetaAlign` column end-align (≥ **0.4.133**; brief start-ink **0.4.144–0.4.147**; **end-ink restored ≥ 0.4.148**) | Shared 17ch column (box starts align). Copy **end**-aligned so short dates sit `end-actions-gap` from `--with-end` actions. Keep `trailingMetaAlign="start"` on date / timestamp catalogs. Live: `#list` org+dates / run-summary. |
| List meta column start-ink (≥ **0.4.144**; **plain trailing text superseded ≥ 0.4.148**) | 0.4.144 briefly start-aligned plain `trailingSupportingText` (regressed Experiences hover). **≥ 0.4.148** restores end-ink for plain meta; `.fynns-list-item-trailing-stats` cells stay **start**-aligned. Drop consumer private `text-align` / width hacks. Live: `#list` org+dates / usage-stats. |
...
| Form ControlRow text-start (≥ **0.4.138**) | When a form host mixes `FieldBlock` + `ControlStack`, ControlRow **label** glyphs inset via `--fynns-form-control-text-inset-inline` (hairline + capsule + field-pad) to match Select / Input **value** text — not shell box left. FieldBlock titles stay flush with the control outer edge. Live: `#field-header`. Re-paste `consumer-cursor-rule.mdc`. |
...
| List run-summary single line (≥ **0.4.141**; gap **0.4.142**; meta start **0.4.143**; trailing end-ink **0.4.148**; overflow **0.4.145**) | Status + identity + duration on **one** horizontal `ListItem` line. Cluster gap = `control-stack-gap` **8dp**. Headline `.fynns-table-meta` = fixed elapse+icon track + **start** align (≥ 0.4.143); **one metric per cell** + label `<span>` ellipsis (≥ 0.4.145). Timestamp → `trailingSupportingText` + **`List` `trailingMetaAlign="start"`** (end-ink ≥ 0.4.148). Drop `InlineAlert`, `lines={2}` stack, between-row `Divider`, jammed multi-metric meta, private status↔title margin, and content-width / start-ink meta that leaves dead space before `--with-end`. Live: `#list` run-summary. |
