...
| Experience / job List meta (≥ 0.4.118; start column token 0.4.119; **opt-in `trailingMetaAlign` ≥ 0.4.120**; **1-icon `--with-end` reserve ≥ 0.4.121**) | Organization under title; dates → `trailingSupportingText`. Mixed-length date catalogs → **`List` `trailingMetaAlign="start"`**. Short status + `--with-end` action catalogs **omit** the prop; lone trash no longer gets a two-disk pad (status↔action ≈ `end-actions-gap`). Live: `#list` org+dates / status+action. Re-paste `consumer-cursor-rule.mdc`. |
