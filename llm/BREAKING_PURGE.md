...
| Mode drawer hide-builtin preference (≥ 0.4.87; **label inset ≥ 0.5.137**) | No **`ControlBlock` + long `description`** in narrow nav tools (~224px → ~102dp stack). Use one-row **`ControlRow` + `InfoHint size="sm"` + track-only `Switch`**. Core ≥ **0.5.137** pads that ControlRow with Item `item-pad-inline-*` so the label is not flush on body `pad-inline` (10dp) alone. Live: Layouts `#layouts-demo-navigation-drawer` + Globals `#info-hint`. Bump-only when the consumer already uses this recipe. |
