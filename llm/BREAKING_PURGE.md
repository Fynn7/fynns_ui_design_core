...
| Editable `CodeBlock` default `rows` | Default is **`1`** (was ~6). Height-resolved fill hosts still stretch the editor; pass an explicit `rows` when the parent height is indefinite and you need a taller resting well. |
