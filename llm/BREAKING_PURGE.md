...
| CodeBlock editable selection / wrap (≥ 0.4.88; continuous soft-wrap wash ≥ **0.4.139**; soft-wrap **single textarea** ≥ **0.5.22**) | **`readOnly`** → single `.fynns-code-block-pre` (full tokens + native `::selection`). Soft-wrap **edit** → **single visible textarea** (Chromium textarea≠pre wrap breakpoints — dual-layer ghost glyphs fixed). `wrap={false}` for live token colors while typing (dual-layer). Live: Globals `#code-block` soft-wrap sample + file-body + readOnly pair. |
