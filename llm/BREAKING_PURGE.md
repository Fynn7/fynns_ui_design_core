...
| `NavigationDrawer` destination gap | Body **destination** siblings (Item / Group / Headline / Divider) share `--fynns-navdrawer-section-gap` (**4dp**). SearchBar / tools ↔ destinations use `--fynns-navdrawer-search-gap` (**4dp**, aliases `section-gap` — same optical row step; was 16dp/`unit-stack-gap` before 0.4.48). Do **not** wrap Item / Group / Headline lists in `.fynns-unit-stack` (that would also 16dp the destination rows). Core remaps `.fynns-nav-drawer-body .fynns-unit-stack` to `section-gap` so mistaken destination wraps still match Group rows — prefer direct children. Re-paste `consumer-cursor-rule.mdc`. |
