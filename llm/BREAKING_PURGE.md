...
| CodeBlock editable selection / wrap (≥ 0.4.88; soft-wrap **visual-line tokens** ≥ **0.5.38** for highlighted edit; **visual-line row breaks + pane resize remeasure** ≥ **0.5.39**; plain soft-wrap `--soft-wrap-single`) | **`readOnly`** → single `.fynns-code-block-pre` (full tokens + native `::selection`). Soft-wrap **edit** + highlight → visual-line overlay (`--soft-wrap-lines`). Soft-wrap edit without highlight → single visible textarea. **`wrap={false}`** → classic dual-layer `<pre>`. Live: Globals `#code-block` soft-wrap sample + file-body + readOnly pair. |
