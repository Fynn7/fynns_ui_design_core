...
- Editable overlay: keyword/module spans **must not** bold against the transparent textarea (soft-wrap drift → wrong-line selection). Highlight scroll re-locks after deferred tokenize; overflow observers stay mounted across keystrokes (content remounts do not reset `scrollTop` every paint). Fill columns: stretch the CodeBlock root (`flex: 1` / definite height); host CSS may set `textarea { height: 100% }` — core does not put percentage height on the editor (collapses when parent height is indefinite). Default `rows={1}`; pass a larger `rows` for non-fill demos.
