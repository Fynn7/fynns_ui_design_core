 ...
 *
 * Crowding checks attach a `MutationObserver` to the shell root — appending a
 * temporary probe there re-fires the observer → `check` → another probe
 * (layout thrash every frame while `navMode="drawer"`).
 */
function resolveLengthPx(host: Element, raw: string): number {
  const v = raw.trim();
  if (!v) return 0;
  if (v.endsWith("px")) {
  ...
  /* Exotic `calc()` etc.: measure under body, never under the observed shell. */
