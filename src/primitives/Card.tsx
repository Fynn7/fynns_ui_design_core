 ...
 *
 * **Width (hard ≥ 0.5.141 / 0.5.142 / 0.5.186):** Card is `width: 100%` /
 * `align-self: stretch` of its parent. On PageScroll destination canvases the
 * parent is `.fynns-content-column` (fills the scrollport; inset pad only —
 * no soft ~1180dp reading ceiling). A section ControlRow (e.g. tool
 * ToggleGroup) and sibling Cards must share that column’s **right edge** —
 * do **not** wrap Cards in `sheet-max-width` (BottomSheet), `chat-max-width`
 * (Chat host only), or `dialog-max-width*` (Dialog panels). Live: sandbox
 * `#page-scroll`.
 *
