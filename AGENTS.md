...
- **DON'T** put **labeled** `Button` / `Select` (or other wide inspector chrome)
  ...
  **0.5.67** widens meta|CTA|Select cell gaps to **8dp** (`inspector-end-gap`
  — not 4dp cluster kiss). Still keep the cluster `nowrap` and short labels. Live:
...
- **Layout helpers:** ControlStack, ControlRow, **ControlBlock** `{ children,
  ...
  **Content density (data → component — hard for agents):** Pick the keep-set
  ...
  | **Inspector row: kind Select ± gap CTA** (catalog line + remap / assist) | Same `ListItem`: overline / headline + optional short `trailingSupportingText` + trailing `.fynns-control-cluster` of optional labeled `Button` `sm` + `Select` (content-hug). Core ≥ **0.5.54** pins in-flow; ≥ **0.5.55** end pad ≥ `radius-3xl`; ≥ **0.5.56** gap meta in end strip; ≥ **0.5.67** meta|CTA|Select gaps **8dp** (`inspector-end-gap`). Leave `trailingMetaAlign` unset. Live: `#list` inspector trailing | Select+Button overlay stack; Select clip; meta drift; 4dp kiss between meta/CTA/Select; private position hacks |
