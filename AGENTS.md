...
- **DON'T** stack **three or more** labeled **`Button`s** in one narrow
  ...
  **information redundancy** and wastes vertical rhythm. **One** primary labeled
  ...
  (`iconOnly` `DownloadIcon` trigger + menu items, or a single short export
...
- **Layout helpers:** ControlStack, ControlRow, **ControlBlock** `{ children,
  ...
  **Content density (data → component — hard for agents):** Pick the keep-set
  ...
  | **Narrow EndAside / inspector Card action row** (generate + save + multi-format export) | `ControlRow` **`label=""`** when **`Card` `title`** names the section — full-width controls band; **one** primary labeled `Button`; save / export → **`IconButton` + `Tooltip`** and **`DropdownMenu` `iconOnly`** — **`ghost` `sm`** (omit `variant`; do **not** `tonal` disks unless user asked). Exports → **one** menu. Live: Layouts `#layouts-demo-shell` aside Card | Visible `ControlRow` label crushed beside cluster; **tonal** save/export icons competing with primary fill |
