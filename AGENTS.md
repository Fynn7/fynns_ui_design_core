...
- **DON'T** put **labeled** `Button` / `Select` (or other wide inspector chrome)
  ...
  **0.5.72** grid trigger-band row + expanded copy `flex-start` (height-3 copy
  ...
  `#list` inspector trailing. **Card / Collapsible head** Select + labeled
  Button in `actions` uses the same trigger-band grammar (≥ **0.5.73**;
  ≥ **0.5.74** collapsed header rhythm + expanded-only grid band) — never
  center title / CTA on the expanded panel. Live:
  `#sandbox-card-head-select`. Failure mode:
  [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **List trailing Select
  + labeled Button overlap** / **Card head Select trigger-band**.
...
- **DON'T** park **`InlineAlert` / `Banner`** beside a **`List`** / catalog in
  the same **row** of a multi-column **`ControlStack`** — severity strips
  belong **above** the first catalog row (full-width sibling in
  `.fynns-unit-stack`, or `ControlStack` with only `ControlRow` children).
  Core ≥ **0.5.120** spans non-`ControlRow` stack children full width, but
  consumers must still prefer **unit-stack**: alert → `List` → optional
  `ControlRow` foot. Live: `#env-check`. Failure mode:
  [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **InlineAlert beside
  List in ControlStack**.
...
- **DON'T** put **runtime / service status** on both `ControlRow` `label` **and**
  a `Chip` in the same service control row — **information redundancy**. Status
  lives on the **label only** (`PID …` when running; `Not running` / `未运行`
  when stopped). The cluster is **labeled `Button`s only** (Start / Stop /
  Restart) — **no** status `Chip`. Live: `#rhythm` service control. Failure
  mode: [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **service control
  status Chip + label**.
- **DON'T** invent private `*-control-cluster` / `hub-spread` flex hosts with
  ...
  `.fynns-control-cluster` (core ≥ **0.5.80** widens labeled-Button siblings
  to **8dp** / `action-cluster-gap`; end-align foot strips use
  `.fynns-control-cluster--end-align` — same as Dialog foot). Drop consumer
  CSS clones of the cluster. Live: `#rhythm` service control + end-align.
  Failure mode:
  [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **private labeled Button
  cluster gap**.
- **DON'T** leave **`IconButton` `loading`** showing the spinner **and** the
  glyph in the same disk (spinner + refresh overlap). Core ≥ **0.5.80** renders
  spinner-only for `iconOnly` — labeled `Button` `loading` still pairs spinner +
  label text. Live: `#rhythm` morph strip.
- **DON'T** use **`IconButton` `size="sm"`** on **PageScroll section-body**
  `ControlRow` strips when the action scope is the **whole section below**
  (copy full posting, re-extract whole posting, save/export whole letter /
  skills doc, reset entire highlight stage) — default **`md`** (40dp ghost
  disk). **`sm`** stays for field-local chrome (Card `actions` beside sm
  peers, EndAside inspector, mode drawer `--toolbar-end`, FieldBlock label-row
  add/remove, Select-row refresh beside sm `InfoHint`). **Same destination
  page:** catalog `ControlRow` + List trailing + section-body strip share
...
- **DON'T** use visible **Card-body** `FieldHint` or multi-sentence
  ...
  **Card** `actions` (≤1 per head) for field- or section-level policy.
  ...
  the `Input`. Live: `#sandbox-field-header-env-keys`. Failure mode:
  [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **env key FieldHint under
  input**.
...
- **DON'T** mount a **full-workflow** `Card` (switches, tabs, CodeBlock,
  ...
  zero inset**.
- **DON'T** wrap a **tall multi-field** `FieldStack` on **`PageScroll`** (roughly
  **≥3 `FieldBlock`s**, or a body tall enough to push the page’s primary
  `ControlRow` / editor below the first viewport) in a static **`Card`** —
  use **`Collapsible`** (`defaultOpen` while the user is filling; same
  `title` / `FieldStack` grammar). **Default:** save / sync IconButtons live
  in the **body** after `FieldStack` — one `.fynns-control-cluster--end-align`
  of **default `md`** ghost disks (section-scope, same band as PageScroll
  generate strips). **Optional head `actions`** only when the product
  explicitly wants compact title-bar chrome → **`sm` only** — **never `md`**
  on Collapsible head. **Card** stays for **List** catalogs, short
  always-visible sections (≤2 fields), and Dialog hosts. Live:
  `#form-recipe-page-scroll`. Failure modes:
  [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **PageScroll brief Card**
  / **Collapsible brief save on head**.
...
- **DON'T** let standalone **`ControlRow`** labeled **`Button`** clusters overflow
  ...
  keep nowrap end-hug). Core ≥ **0.5.92** also raises
  `--fynns-layout-end-aside-min-width` to `clamp(17.5rem, 32%, 22rem)` so drag /
  flex cannot crush the pane below inspector Card labeled-button rows. Live:
  Layouts `#layouts-demo-shell` aside Card. Failure mode: [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **ControlRow labeled
  Button cluster overflow**.
- **DON'T** stack **three or more** labeled **`Button`s** in one narrow
  **EndAside** / inspector **`ControlRow`** cluster (save + export Word + export
  PDF beside the primary CTA) — even when core wrap prevents clip, it reads as
  **information redundancy** and wastes vertical rhythm. **One** primary labeled
  `Button` max; secondary save / sync → **`IconButton` + `Tooltip`** (`SaveIcon`,
  short tip); **variant exports** (Word / PDF / …) → **one** `DropdownMenu`
  (`iconOnly` `UploadIcon` trigger + menu items, or a single short export
  label) — not parallel tonal pills. Live: Layouts `#layouts-demo-shell` aside
  Card. Failure mode: [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md)
  **EndAside dense labeled Button strip**.
- **DON'T** put a visible **`ControlRow` `label`** on a narrow **EndAside** /
  inspector **`Card` body** action strip when the **`Card` `title`** already names
  the section (cover letter / skills / …) — the label column (`1fr | controls`)
  crushes to a few px beside a primary labeled `Button` + icon cluster; use
  **`label=""`** so core paints the full-width controls band (empty label →
  single column; live sandbox aside Card). Section scope stays on **`Card`
  `title`** + `FieldHint` — not a second muted caption on the action row.
  Failure mode: [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **EndAside
  ControlRow label crush**.
- **DON'T** paint secondary **`IconButton`s** / **`DropdownMenu` `iconOnly`**
  triggers in narrow **EndAside** / inspector **`Card` body** clusters as **`tonal`**
  (or `primary` / `elevated`) disks unless the user **explicitly** asked for filled
  chrome — default **`ghost`** (`IconButton` default; `iconOnly` menu defaults
  `ghost` when `variant` omitted). One **primary** labeled `Button` carries the
  main CTA fill; save / export / sync icons stay ghost beside it. Live: Layouts
  `#layouts-demo-shell` aside Card. Failure mode:
  [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **EndAside icon tonal disks**.
- **DON'T** put **`PageScroll`** in **`EndAside`** without the core height chain
  (track `display:flex` + `max-height:100%`, aside `height:100%`, PageScroll
  `flex:1` `min-height:0` `overflow-y:auto` — not `overflow:hidden` on the
  wrapper). Tall inspector catalogs must scroll inside the pane; never grow the
  aside to full content height with a dead overlay rail. Core ≥ **0.5.93**. Live:
  Layouts `#layouts-demo-shell` aside PageScroll. Failure mode:
  [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **EndAside PageScroll no
  scroll**.
...
- **DON'T** pass `drawerHeadline` / `NavigationDrawer` `headline` on
 **DestinationAppShell** or any destination **`ClippedNavShell` with
 `TopAppBar` `title`** — the muted `.fynns-nav-drawer-headline` band stacks a
 second title under the app bar and wastes drawer height on flat root
 destination lists (`NavigationDrawerItem` labels already name each
 destination). **Omit** the prop entirely — live Layouts
 `#layouts-demo-shell`. Reserve sheet `headline` for **standalone**
 `NavigationDrawer` anatomy demos **without** TopAppBar (sandbox Globals
 `#globals-demo-navigation-drawer` only). For body section labels inside a
 rich drawer, use `NavigationDrawerGroup` (collapsible) — not a sheet
 headline + flat items. Failure mode:
 [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **drawer sheet headline
 under TopAppBar**.
...
- **Layout helpers:** ControlStack, ControlRow, **ControlBlock** `{ children,
  ...
  **Content density (data → component — hard for agents):** Pick the keep-set
  ...
  | Expandable catalog (session / turn tree + nested records) | Same `List`: **direct** `ul > li`. Timestamp / kind → `overline`; name → `headline`; path → `supportingText`; duration / count → `trailingSupportingText` (duration units **spaced**: `1m 47s`, never `1m47s`). **Project group count** (`0/4`) → `trailingSupportingText` / `.fynns-table-meta` — **never** headline-tail flex beside `--with-end` IconButtons (≥ **0.5.92** reserves 3× `sm` disks). **Multi-metric trailing** (elapse \| tokens \| cost \| count that must **column-align across rows**) → `.fynns-list-item-trailing-stats` (fixed CSS grid tracks; `--pair` for duration + count only) — **not** a flex `.fynns-control-cluster` (content-width cells drift per row). Expand = row `onClick` + `ChevronRight` leading + `aria-expanded` (core rotates + `.fynns-expand` morph ≥ **0.5.91** — keep `detail` mounted; do **not** `open ? … : null`; do **not** `selected` for open). Nested `List` or `.fynns-table-wrap.fynns-scroll` → `ListItem` **`detail`**. **Parent row ↔ nested members:** host `--with-detail` column `gap` = `--fynns-list-item-gap` (**4dp**, ≥ **0.5.110**) — parent pill must not kiss the first child. Parent **`trailing` `--with-end` IconButtons** dock to the **row shell** only (≥ **0.5.66** — not vertically centered on parent+detail host). Open-record / group actions → trailing `IconButton` cluster (same row chrome). Long copy → `Tooltip` on headline; core ellipsizes. Live: sandbox `#list` expandable tree + project group | `ul > div` around items (**Collapsible / Card / unit-stack as List children** — illegal nesting; flex-shrink + `overflow: hidden` crushes groups to ~19px “skeleton” pills); orphan `ListItem` outside `List`; raw `<button>` / `HubTreeDisclosure` in `leading`; `ControlRow` in trailing; `Button` in headline; **headline-tail count** beside `--with-end` icons (overlap); nested table `width:100%` + `table-layout:fixed` + `overflow-x:hidden`; cell or headline `Chip` as status; consumer `width: max-content` / `tip-grow` on headline (breaks `…`); trailing IconButton as a second highlight island; **`--with-end` actions mid-gap over nested children** (core &lt; 0.5.66); **parent row kisses first nested member** (core &lt; 0.5.110 host `gap: 0` or consumer `margin` hacks on nested `List`); glued duration `1m47s`; flex cluster for multi-stat trailing that must align across rows |
  ...
  | Editable env / config key `FieldBlock` (`.env` / secrets editor) | Label = **key name only**. Portal / format / where-to-copy → **`InfoHint` `size="sm"`** in **`FieldHeader` `actions`** (may sit beside status `Chip`); control = `Input` only — **`FieldHint` only** for validation / save error one-liner, **not** policy under the input. Live: `#sandbox-field-header-env-keys` | `FieldHint` under `Input` for portal / Application ID essays; policy on label row **and** body hint |
...
| **Service / process control row** (Start / Stop / Restart labeled Buttons in Card `ControlRow`) | `ControlRow` `label` = **one** short status string only — **running:** `PID 40812`; **stopped / idle:** `Not running` / `未运行` (not also a Chip). **One** `.fynns-control-cluster` of labeled `Button`s `sm` **only** — **no** status `Chip` beside the actions (information redundancy). Core ≥ **0.5.80** widens labeled-Button gaps to **8dp** (`action-cluster-gap`). **At most one** `loading` in the cluster. Live: `#rhythm` service control | Private `*-control-cluster` at 4dp only; `margin` hacks on `.fynns-btn`; **label + Chip both naming status** (`未运行` + `已停止`, `Not running` + `Stopped`, or label `-` + Chip `Stopped`) |
...
| **Mode drawer SyncSideFilter** (compact `ToggleGroup` between toolbar and catalog) | **Omit option `tip`** (keep `ariaLabel` / visible labels) — ≥ 0.4.97. **`showCheck={false}`** when labels are mark glyphs (C / O) — default check slot + optical translate used to bleed into “全部” (≥ 0.4.100 also clips chips). Body sibling gap to tools / destinations = `--fynns-navdrawer-search-gap` (**8dp**, ≥ 0.4.98 — not Item `section-gap` 4dp). Live: Layouts `#layouts-demo-navigation-drawer` | Any `tip` collides with sort/+ or EmptyState; tools↔filter crushed to 4dp `section-gap`; default `showCheck` wash bleed onto next mark |
  | **Draft discard / save on an editable detail** (reload-from-disk + write file) | **`Card` `actions`** on the Card that **owns** the editable subject (file body `chrome="plain"`, connection fields Card, settings Card). Flat `.fynns-control-cluster` of `IconButton` `ghost` `sm` (CatalogMorph) + `Tooltip`; optional view-diff when dirty. Live: `#sandbox-card-draft-actions` / `#card` | Bare `.fynns-control-cluster--end-align` or `ControlRow` of save/discard IconButtons as a **PageScroll / unit-stack sibling outside any Card** (floating chrome — scope unclear); parking draft chrome only under a `FieldHint` essay with no Card title |
  | **PageScroll multi-field brief** (optional angle / prefs survey before main workflow) | **`Collapsible`** as first sibling in `.fynns-unit-stack` (`title` + `FieldStack` body). **`defaultOpen`** while filling; user collapses to reach generate `ControlRow` / editor below. **Default save / sync:** body `.fynns-control-cluster--end-align` after `FieldStack` — **default `md`** ghost `IconButton`s + `Tooltip` (≥ **0.5.109** — section-scope, not field-local `sm`). **Optional head `actions`** only when the product explicitly wants title-bar chrome → **`sm` only** — **never `md`** on Collapsible head. Same FieldStack tree as `#form-recipe`. **Card** only when the section wraps a **List** or stays **short** (≤2 `FieldBlock`s). Live: `#form-recipe-page-scroll` (≥ **0.5.108** shell; ≥ **0.5.109** body actions) | Static **Card** with 4+ `FieldBlock`s (~600dp+) stacking above generate strip / letter body on PageScroll — no collapse; save/sync IconButton in **Collapsible head** (default recipe — use body end-align **md**); **`md`** IconButton in Collapsible head |
  | Titled section shell | One **`Card`** or **`Collapsible`** (`title` / optional `icon` / `actions`) wrapping the **list or form** — on **PageScroll**, prefer **`Collapsible`** when the section is a **tall multi-field brief** (see row above); **`Card`** for **List** catalogs and **short** always-visible blocks. `title` = **short section name only** — counts / branch / path → **body** (`.fynns-table-meta` / mono), never a title `fynns-control-cluster` of name+tally (glued `OpenSpec8`). Header `actions` = **interactive chrome only** (one horizontal `.fynns-control-cluster` of `IconButton` / `Button` / **at most one** `InfoHint` — nowrap under `.fynns-card-actions`); nested helper clusters hug + nowrap. Prefer **flat** IconButtons in `actions` (not nested clusters). **Do not** park ToggleGroup / labeled segmented controls in Card `actions` (move to body ControlRow / FieldBlock; overflow secondary icons into DropdownMenu iconOnly More). Lead prefers ~40% flex-basis but **yields** (min-width: 0) so a dense IconButton strip never clips under Card overflow: hidden (≥ **0.5.57**) — long titles ellipsize. **InfoHint density (hard):** ≤1 help “i” in `actions`; `content` = **short tip** (≈1–2 sentences / ≤~120 chars preferred) — never twin InfoHints (include vs exclude) and never multi-topic essays in the bubble; longer / split topics → Card **body** `FieldHint` / `InlineAlert` / unit-stack bullets. **Form trailing chrome column (hard — not icons only):** in one strip (`.fynns-top-app-bar-trailing` / leading+trailing of one TopAppBar, `.fynns-card-actions`, one `.fynns-control-cluster` of icon chrome, **or the same destination page’s catalog `ControlRow` + List `trailing`**) **and** in one Card **body trailing chrome column** that spans components (`FieldBlock` / `FieldHeader` `actions` InfoHint, `.fynns-control-cluster--end-align` refresh `IconButton`, ControlRow trailing InfoHint **or lone `.fynns-table-meta`**) every trail cell shares the **same** box size — IconButton / icon `InfoHint` use one `size`; lone status meta (`-` / `OK` / `Fail` without a sibling tip) occupies that same trail box (core ≥ **0.4.137** centers the glyph in `--fynns-form-trail-chrome`). End-docked cells share **one vertical end edge** (same size ⇒ same left edge too). Think grid columns: Select+refresh and probe rows’ trailing cells align as one trail track — not “icons lock” while meta floats as a thin end glyph. Default page chrome / TopAppBar = **`md` (40dp)**; dense form field headers / probe rows = **`sm` (32dp)** — when the label-row InfoHint is `sm`, the Select-row refresh and ControlRow InfoHints in that Card stay `sm` (never default `md` refresh under `sm` InfoHint). Dense Card heads may use `sm` only when **every** icon in that head is `sm`. Never mix `sm` + `md` in the same strip or the same Card trailing column (e.g. InfoHint `sm` at left≈810 beside refresh `md` at left≈802). **CatalogMorph / mode-sidebar detail** (drawer `--toolbar-end` already `sm`): Card `actions` IconButtons / icon SplitButtons **must** be `size="sm"` (32dp) so head chrome matches the mode tools — default `md` (40dp) overfills the Card head (agents-hub workflows lesson ≥ **0.5.62**). Root destination pages without mode tools still prefer TopAppBar-matched Card head `md`. Live: `#card` actions strip + `#sandbox-card-draft-actions` + `#field-header` (label InfoHint + refresh + probe OK/Fail + idle `-` meta all on the `sm` trail) + `#list` / `#rhythm` catalog strip. | Card/Surface **inside** each ListItem; nested `width:100%` / wrap clusters stacking IconButtons into a ~72dp tall head; raw path/branch/count in `title` or `actions`; narrow host that leaves the lead ~one glyph wide; **two+ InfoHints** in one head; long paragraph tips; **mixed 32/40 trail cells** (icons or lone `.fynns-table-meta`) in one bar, one Card head, **or one Card body trailing column**; **orphan draft save/discard end-align strip outside any Card** |
  | **Card head Select + labeled Button** (revision + regenerate) | `Card` `actions` = `.fynns-control-cluster` of content-hug `Select` + `Button` `sm`; inline `search-bar--expanded` (≥ **0.5.70**); ≥ **0.5.73** pins **title + CTA on the 40dp trigger band** when expanded; ≥ **0.5.74** keeps **collapsed** default head center + lead stretch; ≥ **0.5.75** Select↔labeled Button gap **8dp** (`action-cluster-gap`). Live `#sandbox-card-head-select` | Title / Button centered on expanded Select height; flex-start / lead pad when collapsed; **4dp** Select↔Button kiss; private margin hacks |
  ...
  | **Time-series / combo analytics** (stacked volume + secondary metric line) | **`Card`** title + body **`ControlRow`** + compact **`ToggleGroup`** view switch (not Card `actions`) + `.fynns-chart` plot (min-height `--fynns-chart-plot-min-height`) + bottom legend. Stacked bars → left Y (`--fynns-chart-series-1` / `series-2`); line → right Y (`--fynns-chart-series-line`; right tick ink matches line). Horizontal grid only (`--fynns-chart-grid-stroke`); axis lines off; ticks `font-size-xs` + `text-muted`. **Connected line: no idle vertex dots** — hover / active column only; **gentle monotone curve** at turns (Fritsch–Carlson / Recharts `monotone` — **not** Catmull-Rom; non-negative series must not dip below 0 between samples); `strokeLinecap` / `strokeLinejoin` `round`. Discrete USD markers only on hover. Hover flyout → **`.fynns-chart-tooltip`** (`__title` at `tooltip-title-size` / sm, metric rows at `tooltip-body-size` / xs, `tooltip-row-gap` 4dp, `tooltip-pad-block` 12dp — **not** `.fynns-unit-stack` or `.fynns-control-cluster` rows) that **follows the pointer** (pointer coords from the chart wrapper on `onMouseMove` → Tooltip `position` — do not lock Y to the active series value). **Hand-drawn charts:** outer **`.fynns-chart-tooltip-shell`** + **`clampChartPointerTooltipBox()`** (top-left box — no `translate(-50%)` centering) so the full flyout stays inside the plot host near edges. Keep a **stable tip size** while moving (do not clear the box / fall back to a smaller estimate on every `mousemove` — that jitters edge clamp). Prefer **column-center X** + pointer **Y** for category tips. Line-metric row → `__row--line`. Colors/tokens: `CHART_TOKENS` / `--fynns-chart-*`. Live sandbox `#chart` (Recharts in sandbox devDeps only — not a keep-set export). | Neon orange line on dark teal; **dense idle line dots**; **sharp polyline or sub-zero spline overshoot**; **tooltip locked to series-value Y**; **tooltip clipped at plot edge** (`translate(-50%)` + naive `clamp(left)` only); **tooltip edge clamp jitter** (estimate↔measure thrash / raw pointer X in edge band); **unit-stack / 16dp gaps inside chart tooltip**; title same size as body with crushed block pad; vertical grid; `ToggleGroup` in Card `actions`; rainbow series; axis lines + tick marks; hardcoded hex in consumer CSS; pasting consumer product chart copy into Globals |
