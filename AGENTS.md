...
- **DON'T** put **stopped / idle runtime state** on both `ControlRow` `label`
  (`Not running` / `未运行`) **and** a `Chip` `assist` in the same service
  control row (`Stopped` / `已停止`) — **information redundancy**. **Running:**
  label = `PID …` only; Chip = `Running`. **Stopped:** label = ASCII **`-`**
  (probe empty slot); Chip carries the state word. Live: `#rhythm` service
  control. Failure mode: [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md)
  **service control twin stopped copy**.
...
| **Service / process control row** (status Chip + Start / Stop / Restart labeled Buttons in Card `ControlRow`) | **Running:** `ControlRow` `label` = short runtime meta only (`PID 40812` — not the state word) + **one** `.fynns-control-cluster` of `Chip` `assist` (`Running` / `运行中`) + labeled `Button`s `sm`. **Stopped / idle:** `label` = ASCII **`-`** (no PID yet) — **never** also print `Not running` / `未运行` beside a `Stopped` / `已停止` Chip (information redundancy). State lives on the Chip only when idle. Core ≥ **0.5.80** widens labeled-Button gaps to **8dp** (`action-cluster-gap`). **At most one** `loading` in the cluster. Live: `#rhythm` service control | Private `*-control-cluster` / bare flex with `control-cluster-gap` only (~4dp kissed pills); inventing `margin` on `.fynns-btn` instead of the public cluster class; **twin stopped copy** on label + Chip (`未运行` + `已停止`, `Not running` + `Stopped`) |
