...
- **DON'T** put **labeled** `Button` / `Select` (or other wide inspector chrome)
  ...
  **0.5.54** **pins** that end cluster **in-flow** (always visible; Select
  `width: auto`) — still keep the cluster `nowrap` and short labels. Live:
...
- **Layout helpers:** ControlStack, ControlRow, **ControlBlock** `{ children,
  ...
  **Content density (data → component — hard for agents):** Pick the keep-set
  ...
  | **Inspector row: kind Select ± gap CTA** (catalog line + remap / assist) | Same `ListItem`: overline / headline + optional short `trailingSupportingText` + trailing `.fynns-control-cluster` of optional labeled `Button` `sm` + `Select` (content-hug). Core ≥ **0.5.54** detects Select / non-icon Button and **pins** the end cluster in-flow (no IconButton overlay / no 40dp reserve stacking). Prefer short CTA copy; long workflows still open a `Dialog`. Live: `#list` inspector trailing | Treating Select+labeled Button as path-catalog hover IconButtons (absolute overlay + `Select` `width:100%` → CTA text paints onto the trigger); inventing private `position` / negative margin; pinning core &lt; 0.5.54 |
