...
- **Layout helpers:** ControlStack, ControlRow, **ControlBlock** `{ children,
  ...
  **Content density (data → component — hard for agents):** Pick the keep-set
  ...
  | **Morph strip: IconButtons + labeled CTA** (posting re-extract beside refresh + copy) | Standalone `ControlRow` + `.fynns-control-cluster` of `sm` ghost `IconButton`s + **one** labeled `Button` `sm` **last (rightmost)** — `tonal` / `primary` / secondary labeled CTA all follow the same **primary-end** grammar (≥ **0.5.105**; same as Dialog feet / mode drawer). Icon↔icon **4dp**; icon→labeled **8dp** (≥ **0.5.80**). **`IconButton` `loading`** = spinner-only in disk (no glyph overlap); **one** `loading` in cluster. When a one-line **content** hint follows, wrap strip + `FieldHint` + body in `.fynns-unit-stack` (≥ **0.5.104**). Live: `#rhythm` morph + hint stack | Spinner on top of refresh glyph; 4dp kiss before labeled pill; twin loading on refresh + import; bare `ControlRow` + sibling `FieldHint` (zero gap); **labeled primary / generate parked leftmost** before save/export icons |
  | **ControlRow chrome strip: IconButtons + labeled primary** (cover letter / CV skills generate beside save / export / prompt) | Standalone `ControlRow` + `.fynns-control-cluster`: **`sm` ghost `IconButton`s + `DropdownMenu` `iconOnly` first** → **one labeled `Button` `primary` (or main tonal CTA) last (rightmost)** — never primary leftmost before secondary icons (≥ **0.5.105**). Same LTR grammar as morph strip / Dialog feet / mode drawer New. Icon→labeled **8dp** when labeled is last. Live: `#rhythm` cover-letter strip + morph stack | Primary generate/run **first** in cluster; save/export/prompt icons after the main pill; private `margin-left: auto` hacks |
