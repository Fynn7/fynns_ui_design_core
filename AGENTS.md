...
- **DON'T** use visible **Card-body** `FieldHint` or multi-sentence
  ...
  **Card** `actions` (≤1 per head) for field- or section-level policy.
  ...
  status `Chip` (`已配置` / `Missing`). Live: `#sandbox-field-header-env-keys`.
  Failure mode: [`llm/CONSUMER_TREATY.md`](llm/CONSUMER_TREATY.md) **env key
  FieldHint under input** / **env key status Chip**.
