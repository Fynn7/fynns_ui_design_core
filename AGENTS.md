...
- **Layout helpers:** ControlStack, ControlRow, **ControlBlock** `{ children,
  ...
  **Content density (data → component — hard for agents):** Pick the keep-set
  ...
  | **Morph strip: IconButtons + labeled CTA** (posting re-extract beside refresh + copy) | Standalone `ControlRow` + `.fynns-control-cluster` of `sm` ghost `IconButton`s + **one** labeled `Button` `sm`. Icon↔icon **4dp**; icon→labeled **8dp** (≥ **0.5.80**). **`IconButton` `loading`** = spinner-only in disk (no glyph overlap); **one** `loading` in cluster. When a one-line **content** hint follows, wrap strip + `FieldHint` + body in `.fynns-unit-stack` (≥ **0.5.104**). Live: `#rhythm` morph + hint stack | Spinner on top of refresh glyph; 4dp kiss before labeled pill; twin loading on refresh + import; bare `ControlRow` + sibling `FieldHint` (zero gap) |
