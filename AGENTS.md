...
- **Layout helpers:** ControlStack, ControlRow, **ControlBlock** `{ children,
  ...
  **Not** List pills, ChatActivity, Tree, or lettered A/B/C / list-detail
  ...
  so flat and detail headlines share one start edge. Disc / node mid =
  **headline** line box (`--fynns-timeline-node-band`, ≥ **0.5.14**) — not
  mid title+supporting. Tokens `--fynns-timeline-*`. Live `#timeline`;
