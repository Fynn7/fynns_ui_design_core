...
同一任务内完成 core 约束 → sandbox 可教样例 → 发布 → 上报消费仓验证 → **全消费仓舰队扫描同类问题并清理**。
...
- 闭环见 skill（含步骤 **8 全消费仓舰队扫描**）；消费仓上报的 bug 必须 sandbox + reporting consumer **双端 browser 验证**，再并行扫清其余消费仓同类违规
- 子代理仅 Auto；见用户 rule「子代理仅使用 Auto 模型」
- 浏览器验证见 `.cursor/rules/browser-verify-ui.mdc`
- Parent 留在 `fynns_ui_design_core`；消费仓工作一律 Task 子代理
