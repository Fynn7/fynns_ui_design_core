...
const en = {
  ...
  "globals.busyRegionFieldHelp":
    "FieldBlock body wait + label-row refresh: **BusyRegion only** while busy — header IconButton stays the glyph and is `disabled` **without** `loading`. Never stack BusyRegion ring with a chrome `loading` spinner on the same host (information redundancy). Per-control wait with no BusyRegion may still use IconButton `loading` alone. Live sample below.",
...
const zh: Record<MessageKey, string> = {
  ...
  "globals.busyRegionFieldHelp":
    "FieldBlock 正文等待 + 标签行刷新：busy 时 **只** 用 BusyRegion — 头栏 IconButton 保持字形、仅 `disabled`、**不** 画 `loading`。禁止同一宿主叠 BusyRegion 圈与 chrome `loading` 第二圈（信息冗余）。无 BusyRegion 的单控件等待仍可用 IconButton `loading`。下方为活样例。",
