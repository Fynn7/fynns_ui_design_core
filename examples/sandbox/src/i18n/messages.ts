...
const en = {
  ...
  "layouts.fillColumnGuideBody":
    "Describe one real project preference you already keep in notes — region, digests, or a compact mode — without inventing employers.",
...
const zh: Record<MessageKey, string> = {
  ...
  "layouts.fillColumnGuideBody":
    "写一条你笔记里已有的真实项目偏好——地区、摘要节奏或紧凑模式——不要编造雇主。",
  ...
  "layouts.fillColumnGuideAssistant":
    "下一条回答保持简短。表单下方是引导步骤。",
  "layouts.fillColumnGuideHelp":
    "**FillColumn 引导铬（≥ **0.5.277**）：** 短 soft `Surface` 表单 → `FillColumn` **`header`**；`Chat`（可选 `ChatActivity` thinking）→ `children`。Core 用 `unit-stack-gap`（16dp）拉开 header↔main 与 main 内兄弟。**禁止**把表单当 Chat 的 `children` 兄弟并设 `paddingBottom: 0`（Surface 贴死 ChatActivity）。对照 `#layouts-demo-fill-column` 引导舞台。",
