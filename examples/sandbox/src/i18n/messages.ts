...
const zh: Record<MessageKey, string> = {
  ...
  "globals.activityHelp":
    "**ChatActivity** — Wave 2 多步骤 Agent / tool 调用链（Cursor 式状态树）。可折叠标题 + 竖向 rail + `ChatActivityStep` 行。每步逻辑行需要稳定 `key`，避免行身份变形。每步 `icon` 为任意 ReactNode — 本样例用 File / Pencil / Wrench / Search / FolderOpen；省略 → 默认扳手（`done`/`pending`）或状态点（`active`）；`null` → 空节点。流式时瞬间完成的 `done` 步仍会先播状态点，最短 `--fynns-chatmessage-activity-step-min-busy`，再以 `--fynns-chatmessage-activity-complete` 播完成图标与胶囊。新步进入整行 fade（`--fynns-chatmessage-activity-enter`，与 `duration-slow` 高度形变同拍；无上下弹）。可选 `ChatActivityArtifact` 胶囊 + 说明。经 `ChatMessage.thinking` 挂载（可单独或与 `ChatThinking` 并用）。单块推理仍用 `ChatThinking` — 不要把它塞成时间线。几何：`CHATMESSAGE_TOKENS` `activity-*`。",
