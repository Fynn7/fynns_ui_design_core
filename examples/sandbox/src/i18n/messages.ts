...
const zh: Record<MessageKey, string> = {
  ...
  "globals.chartHelp":
    "M3 风格分析图预设：左轴堆叠柱（accent + tonal 堆叠），右轴折线（info）。`ToggleGroup` 视图切换放在 Card 正文 `ControlRow`（勿放进 Card `actions`）。仅水平网格；隐藏轴线；刻度用 `font-size-xs` + `text-muted`；右轴刻度与折线同色。**折线默认不画节点圆点** — 仅 hover 时显示一个 activeDot。**转折处轻微曲线**（Recharts `monotone`；自绘 SVG 用 **Fritsch–Carlson 单调三次**插值，勿用 Catmull-Rom — 非负序列不能弯到 0 以下）。**Tooltip 跟随指针**（在 chart wrapper 上用 `onMouseMove` 取指针坐标设 Tooltip `position` — 勿锁在当前系列值的 Y）。颜色走 `--fynns-chart-*` token。Recharts 仅在 sandbox；消费者复制结构与 CSS，勿粘贴产品文案。",
