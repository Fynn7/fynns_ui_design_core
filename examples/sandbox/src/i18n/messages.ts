...
const zh: Record<MessageKey, string> = {
  ...
  "globals.navDrawerHelp":
    "[自适应] NavigationDrawer — 目的地侧栏（不是内容 Drawer）。`standard` = 中等及以上常驻；`modal` = 遮罩覆盖。Sheet `headline` = **仅静态标题字符串**（本样例「邮件」）— 禁止塞返回 IconButton、批量工具条或两端对齐的裸计数。模式返回 → TopAppBar `leading` / `leadingExtra`（布局模板 `#layouts-demo-shell`）。Group/Item `label` = **仅名称** — 除非用户显式要求，禁止 `· N` 填充；未读等 → 仅在需要时用 Item `badge`。`NavigationDrawerHeadline` = body 内静态**分组**文案；`NavigationDrawerGroup` = 可折叠分组行（leading `icon` 任意 ReactNode）+ 缩进子项；折叠且子项 active 时 trigger 显示选中 pill。兄弟 Item / Group / Headline 间距 = `--fynns-navdrawer-section-gap`（4dp，与 Group 内叶子相同）。SearchBar / 工具条与目的地行 = `--fynns-navdrawer-search-gap`（16dp）。必须是 body **直接**子节点；禁止用 `.fynns-unit-stack` 包目的地列表。通用检查器请用 Drawer。",
