...
const en = {
  ...
  "globals.chatComposerThinkingToggleHelp": "Recipe (≥ 0.5.291): Model Menu → Thinking → Vision → Send on a narrow (~365px) host. Core keeps model Menu ≥ leading-menu-min; ≤26rem shells use icon-only mode pills (Tooltip + aria-label). Do not bury modes in + Menu. Live #sandbox-chat-composer-thinking-toggle.",
...
const zh: Record<MessageKey, string> = {
  ...
  "globals.chatComposerThinkingToggleHelp": "配方（≥ 0.5.291）：窄壳（~365px）上 Model → Thinking → Vision → Send；core 保证模型 Menu ≥ leading-menu-min，≤26rem 时 mode pill 仅图标（Tooltip + aria-label）。勿埋进 + 菜单。活样 #sandbox-chat-composer-thinking-toggle。",
