...
const en = {
  ...
  "globals.listRecipeCatalogHelp":
    "**Named recipe / preset / pack catalog (hard):** one Card wrapping **one** `List` — short one-line `supportingText`; kind/source in `trailingSupportingText` (not `ChipSet` soup); long description / id / evidence → Preview `Dialog` or row `InfoHint`. Row actions = ghost **md** IconButtons (+ Tooltip). Hover / coarse must clear kind meta from those disks (core ≥ **0.5.207**). **Tab focus** (≥ **0.5.218**): host inset focus ring — not a cracked child `outline` under `--with-end` disks. **Forbidden:** fat Card per recipe with stacked `FieldHint` essays + head `ChipSet` tags; meta under IconButtons; page FieldHint that restacks the TopAppBar tip; consumer outline patches on `.fynns-list-item`. Failure: CONSUMER_TREATY recipe catalog fat Card FieldHint essays / List trailingSupportingText underlaps --with-end IconButtons / ListItem Tab focus ring cracked by --with-end overlay. Live `#sandbox-list-recipe-catalog`.",
...
const zh: Record<MessageKey, string> = {
  ...
  "globals.listRecipeCatalogHelp":
    "**命名配方 / 预设 / 打包目录（硬）：** 一个 Card 包 **一个** `List` — 短一行 `supportingText`；种类/来源用 `trailingSupportingText`（勿 `ChipSet` 汤）；长描述 / id / 证据 → Preview `Dialog` 或行内 `InfoHint`。行操作 = ghost **md** IconButton（+ Tooltip）。悬停 / 触控显盘须让开 kind meta（core ≥ **0.5.207**）。**Tab 焦点**（≥ **0.5.218**）：host inset focus 环 — 非 `--with-end` 盘下裂开的子项 `outline`。**禁止：** 每配方一张胖 Card + 叠 `FieldHint` 长文 + 头 `ChipSet` 标签；meta 压在 IconButton 下；栏目 FieldHint 再复述顶栏说明；消费仓自造 `.fynns-list-item` outline。失败：CONSUMER_TREATY recipe catalog fat Card FieldHint essays / List trailingSupportingText underlaps --with-end IconButtons / ListItem Tab focus ring cracked by --with-end overlay。对照 `#sandbox-list-recipe-catalog`。",
