...
const en = {
  ...
  "globals.navDrawerHelp":
    "[adaptive] NavigationDrawer — destination side sheet (not content Drawer). `standard` = medium+ permanent; `modal` = overlay. `NavigationDrawerHeadline` = static section label; `NavigationDrawerGroup` = collapsible folder row (leading `icon` any ReactNode) with indented items; collapsed + active leaf shows selected pill on the trigger. Sibling Item / Group / Headline gap = `--fynns-navdrawer-section-gap` (4dp, same inside Group) — put them as **direct** body children; never wrap destinations in `.fynns-unit-stack` (16dp). SearchBar / tools as a body sibling open `--fynns-layout-unit-stack-gap` (16dp) to the first destination. Prefer Drawer for generic inspector panels.",
