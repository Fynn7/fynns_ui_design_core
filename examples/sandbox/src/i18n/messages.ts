...
const zh: Record<MessageKey, string> = {
  ...
  "globals.navDrawerHelp":
    "[自适应] NavigationDrawer — 目的地侧栏（不是内容 Drawer）。`standard` = 中等及以上常驻；`modal` = 遮罩覆盖。`NavigationDrawerHeadline` = 静态分组文案；`NavigationDrawerGroup` = 可折叠分组行（leading `icon` 任意 ReactNode）+ 缩进子项；折叠且子项 active 时 trigger 显示选中 pill。通用检查器请用 Drawer。",
