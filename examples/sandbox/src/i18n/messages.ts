...
const en = {
  ...
  "globals.fieldHeaderEnvTokenTip":
    "Sample API token — copy from the sample portal Bot settings page.",
  ...
  "globals.fieldHeaderEnvOk": "OK",
  "globals.fieldHeaderEnvMissing": "Missing",
...
const zh: Record<MessageKey, string> = {
  ...
  "globals.fieldHeaderEnvTokenLabel": "SAMPLE_TOKEN",
  "globals.fieldHeaderEnvTokenTip": "示例 API 令牌 — 从示例门户 Bot 设置页复制。",
  ...
  "globals.fieldHeaderEnvOk": "已配置",
  "globals.fieldHeaderEnvMissing": "缺失",
  ...
  "globals.fieldHeaderHelp":
    "**提示压缩：** 分区说明 → Card `actions` `InfoHint`（≤1）；字段策略 → `FieldBlock` `actions` `InfoHint` **或** 标题旁 inline `InfoHint`（≥ **0.5.128**）；重载行为 → 刷新 **Tooltip** — 不要用 Card-body `FieldHint` 或多句 `description`。**可编辑 env 键（硬）：** 标签 = 键名；门户/格式 → **`FieldHeader` `actions` 内 `InfoHint` `size=\"sm\"`** 与状态 `Chip` 并列 — **禁止**在 `Input` 下再写策略 `FieldHint`。Live `#sandbox-field-header-env-keys`。**Input** reveal 用 `trailing`。**Select + 刷新** 用 `.fynns-control-cluster--end-align`（Select `fynns-control-cluster__grow` + 兄弟 `IconButton` **`size=\"sm\"`**，当标签行 InfoHint 为 `sm`）— 不要用 `Select.trailing`。**表单尾部 chrome 列（强制）：** 标签行「i」+ 刷新 + ControlRow 探测「i」**以及** lone `.fynns-table-meta`（`-`）共用同一 trail 盒（标签「i」为 `sm` 时整列 `sm`）— 贴尾共一条竖线；禁止细 meta 贴在 32/40dp 盘下。**标题文字起点（强制）：** ControlRow **标签字形**与 FieldHeader / FieldBlock **标题**左齐（core ≥ **0.5.58**）。Select **值文字**仍在外壳内 inset — 勿再内缩 ControlRow 标签去追值文字。Live `#sandbox-field-header-inline-infohint`。",
