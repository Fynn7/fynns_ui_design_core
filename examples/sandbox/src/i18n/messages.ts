...
const en = {
  ...
  "globals.timelineFlatHelp":
    "Flat Timeline (default catalog): row click → Dialog. Edit + delete live **in the Dialog foot** — **not** `--with-end` hover IconButtons, **not** leading kind glyphs (default disc). Slots: `headline` / org `supportingText` / dates `trailingSupportingText` + `trailingMetaAlign=\"start\"`.",
...
const zh: Record<MessageKey, string> = {
  ...
  "globals.timelineFlatHelp":
    "扁平 Timeline（默认目录）：行点击 → Dialog。编辑 / 删除在 **Dialog 脚** — **禁止** `--with-end` 悬停 IconButton，**禁止** leading 种类图标（默认圆点）。槽位：`headline` / 组织 `supportingText` / 日期 `trailingSupportingText` + `trailingMetaAlign=\"start\"`。",
