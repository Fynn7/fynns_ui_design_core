  ...
  **Chat:** `Chat` / `ChatThread` / `ChatComposer` / `ChatScrollToBottom` /
  ...
  - **Product / session host (default for chat-like UIs):** compose
    ...
    **`FillColumn` → `Chat`**. **New-chat landing:** greeting `EmptyState` +
    soft starters + **`ChatComposer` inside `ChatThread.empty`** (collapse
...
| Chat product / session host | **`ClippedNavShell`** (flat session or drill-in `navKey`; **`topBar={null}`** on new-chat landing — body fills shell ≥ **0.5.261**) + session `NavigationDrawer` (`NavigationDrawerNewChat` + EmptyState\|Items + footer account) + main **`FillColumn` → `Chat`**. **New-chat landing:** vertically centered greeting `EmptyState` + soft interactive Surface starters + **`ChatComposer` inside `ChatThread.empty`** (collapse docked composer row via landing class). Active thread: composer docks under `ChatThread` **inside** Chat. Destination lists stay on `DestinationAppShell` | `#layouts-demo-chat-product` | Trash+Plus twin; Chip / revived ChatStarterPrompts; PageScroll as chat main scroll; composer in FillColumn `footer`; inventing a non–keep-set chat shell |
