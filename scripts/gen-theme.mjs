...
.fynns-scroll-overlay-portal {
   ...
   * below modal-flyout (65) so page rails clear open Select / Menu (>= 0.5.249;
   * was --fynns-z-toast and painted through open menus). Below toast / tooltip. */
  z-index: var(--fynns-z-scroll-overlay);
  pointer-events: none;
  overflow: hidden;
}

/* Flyout host thumbs (Menu / Select list) — second portal above modal-flyout
 * so the panel own rail is not buried under z=65 (>= 0.5.251). */
.fynns-scroll-overlay-portal--flyout {
  z-index: var(--fynns-z-scroll-overlay-flyout);
}
...
/*
 * Soft scroll-edge fade: every non-replaced .fynns-scroll overlay host gets
 * a vertical mask whenever content extends past its visible edge. The shared
 * scrollbar manager measures overflow and writes data-fade-top/bottom; this
 * covers Dialog/Drawer/Sheet bodies, menus, lists, Chat, and PageScroll with
 * no consumer setup. Overlay thumbs stay in the portal (unmasked). Editable
 * CodeBlock and Textarea are replaced elements and keep their own fade path.
 * NavDrawer reads its alias so overriding
 * --fynns-navdrawer-body-fade-length still applies. PageScroll ≥ 0.5.247;
 * FillColumn header ≥ 0.5.278. Table wrap inline-axis ≥ **0.5.296**
...
.fynns-scroll:where([data-fynns-overlay-scroll])[data-fade-bottom],
.fynns-code-block-highlight[data-fade-bottom],
.fynns-fill-column-header[data-fade-bottom] {
  --fynns-scroll-y-mask: linear-gradient(
    to bottom,
    #000 0%,
    #000 calc(100% - var(--fynns-layout-scroll-edge-fade-length)),
    transparent 100%
  );
  -webkit-mask-image: var(--fynns-scroll-y-mask);
  mask-image: var(--fynns-scroll-y-mask);
}
...
.fynns-scroll:where([data-fynns-overlay-scroll])[data-fade-top],
...
.fynns-scroll:where([data-fynns-overlay-scroll])[data-fade-top][data-fade-bottom],
...
/* The content fades inside a bordered scroll well; four opaque strips retain
 * its outer hairline. Unbordered hosts keep the full-bleed mask above. */
.fynns-scroll:where([data-fynns-overlay-scroll])[data-fynns-scroll-border]:is([data-fade-top], [data-fade-bottom]) {
  -webkit-mask-image:
    linear-gradient(#000, #000),
    linear-gradient(#000, #000),
    linear-gradient(#000, #000),
    linear-gradient(#000, #000),
    var(--fynns-scroll-y-mask);
  mask-image:
    linear-gradient(#000, #000),
    linear-gradient(#000, #000),
    linear-gradient(#000, #000),
  ...
  -webkit-mask-size:
    ...
    var(--fynns-border-hairline) 100%,
    calc(100% - 2 * var(--fynns-border-hairline)) calc(100% - 2 * var(--fynns-border-hairline));
  mask-size:
    100% var(--fynns-border-hairline),
    100% var(--fynns-border-hairline),
    var(--fynns-border-hairline) 100%,
    var(--fynns-border-hairline) 100%,
    calc(100% - 2 * var(--fynns-border-hairline)) calc(100% - 2 * var(--fynns-border-hairline));
  -webkit-mask-position: 0 0, 0 100%, 0 0, 100% 0, var(--fynns-border-hairline) var(--fynns-border-hairline);
  mask-position: 0 0, 0 100%, 0 0, 100% 0, var(--fynns-border-hairline) var(--fynns-border-hairline);
...
/*
 * Table wrap — inline-axis soft edge (≥ **0.5.296**). Driven by
 * data-fade-left / data-fade-right from syncScrollEdgeFade (overlay
 * scroll hosts call it on scroll/resize). Live #table.
 */
.fynns-table-wrap[data-fade-right] {
  --fynns-scroll-x-mask: linear-gradient(
    to right,
    #000 0%,
    #000 calc(100% - var(--fynns-layout-scroll-edge-fade-length)),
    transparent 100%
  );
  -webkit-mask-image: var(--fynns-scroll-x-mask);
  mask-image: var(--fynns-scroll-x-mask);
}

.fynns-table-wrap[data-fade-left] {
  --fynns-scroll-x-mask: linear-gradient(
    to right,
    transparent 0%,
    #000 var(--fynns-layout-scroll-edge-fade-length),
...
.fynns-table-wrap[data-fade-left][data-fade-right] {
  --fynns-scroll-x-mask: linear-gradient(
    to right,
    transparent 0%,
    #000 var(--fynns-layout-scroll-edge-fade-length),
...
/* A capped wide table can overflow on both axes; intersect the independent
 * fades so its horizontal mask cannot replace the vertical one. */
.fynns-table-wrap.fynns-scroll:where([data-fynns-overlay-scroll]):is([data-fade-top], [data-fade-bottom]):is([data-fade-left], [data-fade-right]) {
  -webkit-mask-image: var(--fynns-scroll-y-mask), var(--fynns-scroll-x-mask);
  mask-image: var(--fynns-scroll-y-mask), var(--fynns-scroll-x-mask);
